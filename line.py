...
def detect_and_draw_lines(video_source=0):
    ...
    while True:
        ...
        roi = result_frame[height//8:height, width//4:3*width//4]
            ...
            if area > threshold:
                ...
                if area > max_area:
                    ...
                    if len(approx) <= 4  and abs(leftmost[0]-rightmost[0])<=100:
                        ...
                        # 判断十字路口和丁字路口
                        if (abs(leftmost[0]-downmost[0])>alpha and abs(rightmost[0]-downmost[0])>alpha):
                            if abs(topmost[1]-min(rightmost[1],leftmost[1]))>beta:
                                max_shape = "Crossroad: Left, Right, Towards"
                                center[0] = (topmost[0]+downmost[0])//2
                                center[1] = (leftmost[1]+rightmost[1])//2
                                cv2.circle(result_frame, (center[0],center[1]), 5, (30, 255, 255), 2)  # 黄色点
                                bottom[0] = downmost[0]
                                ...
                                max_shape = "T-junction: Left, Right"
                        ...
                        # 判断左转和右转
                            if abs(leftmost[0]-downmost[0]) > abs(rightmost[0]-downmost[0]):
                                if abs(topmost[1]-min(rightmost[1],leftmost[1]))>beta:
                                    max_shape = "Left Forward: Left, Towards"
                                    center[0] = (topmost[0]+downmost[0])//2
                                    ...
                                    max_shape = "Left Turn"
                                    if abs(topmost[0]-leftmost[0])<100 or abs(topmost[0]-downmost[0])>100:
                                ...
                                if abs(topmost[1]-min(rightmost[1],leftmost[1]))>beta:
                                    max_shape = "Right Forward: Right, Towards"
                                    center[0] = (topmost[0]+downmost[0])//2
                                    ...
                                    if abs(topmost[0]-rightmost[0])<100 or abs(topmost[0]-downmost[0])>100:
